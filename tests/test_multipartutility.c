...
int processPacking(char *filename1, char *filename2, uint32_t version, char * subdocname)
{
	...
	if(readFromFile(filename1, &fileData1, &len1) && readFromFile(filename2, &fileData2, &len2))
	{
		...
		if(packRootData != NULL && blobPackSize > 0)
		{
			...
			packRootData->data_items[0].name = strdup("Device.GRE.X_RDK_PublicHotspotData");
			packRootData->data_items[0].value = malloc(sizeof(char) * blobPackSize+1);
			memset(packRootData->data_items[0].value, 0, sizeof(char) * blobPackSize+1);
			packRootData->data_items[0].value = memcpy(packRootData->data_items[0].value, (char *)packedData, blobPackSize+1);
			packRootData->data_items[0].value[blobPackSize] = '\0';
			packRootData->data_items[0].value_size = blobPackSize;
			printf("went here2\n");
...
{	
	...
	if(!parseSubDocArgument(argvector, argument, &subdocs))
	{
		free(rootVersion);
		free(subdocs);
		exit(0);
...
void add_suites( CU_pSuite *suite )
{
    *suite = CU_add_suite( "tests", NULL, NULL );
    CU_add_test( *suite, "test_mul_util", test_mul_util);
}
