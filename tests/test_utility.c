...
int readFromFile2(const char *filename, char **data, size_t *len);
int writeToFile2(char *file_path, char *data, size_t size);
/*----------------------------------------------------------------------------*/
/*                             Internal Functions                             */
/*----------------------------------------------------------------------------*/
static void __msgpack_pack_string_nvp( msgpack_packer *pk,
...
int processEncoding(char *filename, char *encoding, int isBlob)
{
	...
	if(readFromFile2(filename, &fileData, &len))
	{
		...
		if(encodedLen > 0 && encodedData != NULL)
		{
			...
			if(writeToFile2(outFile, encodedData, encodedLen) == 0)
			{
...
int processPacking(char *filename1, char *filename2, uint32_t version, char * subdocname)
{
	...
	if(readFromFile2(filename1, &fileData1, &len1) && readFromFile2(filename2, &fileData2, &len2))
	{
		...
		if(blobPackSize > 0)
		{
			...
			if(writeToFile2(B64OUTFILE, (char *)encodedData, (size_t)encodedLen) == 0)
			{
				fprintf(stderr,"%s File not Found\n", B64OUTFILE);
				free(encodedData);
				return 0;
		...
		if(rootPackSize > 0 )
		{
			...
			if(writeToFile2(OUTFILE, (char *)packedRootData, rootPackSize) == 0)
			{
				fprintf(stderr,"%s File not Found\n", OUTFILE);
				free(packedRootData);
				return 0;
	...
	for (i = 2; i<count; i++)
	{
		...
		if(!multilevelBlob)
		{
			if(processEncoding(fileName1, "M", isBlob))
			{
				...
				if(readFromFile2(outFile, &fileData, &len))
				{
...
int getSubDocBuffer(multipart_subdoc_t subdoc, char **buffer)
{
	...
	bufLength += length;
	pHdr += length;
	length = add_header("Dest: ","webconfig.wifi.privatessid", pHdr);
	bufLength += length;
	pHdr += length;
	length = add_header("Dest1: ","webconfig.wifi.privatessid", pHdr);
	bufLength += length;
	pHdr += length;
	length = add_header("Dest2: ","webconfig.wifi.privatessid", pHdr);
	bufLength += length;
	pHdr += length;
	length = add_header("Dest3: ","webconfig.wifi.privatessid", pHdr);
	bufLength += length;
	pHdr += length;
	*buffer = hdrbuf;
	return bufLength;
...
int main2(int argc, char *argv[])
{
	...
	if(bufLen > 0)
	{
		...
		if(writeToFile2(MULTIPART_DOC, buffer, bufLen) == 0)
		{
			fprintf(stderr,"%s File not Found\n",MULTIPART_DOC);
		}
	}
...
void add_suites( CU_pSuite *suite )
{
    *suite = CU_add_suite( "tests", NULL, NULL );
    //CU_add_test( *suite, "Full", test_main);
}
