...
void test_wan_unpack()
{
	int len=0;
	char subdocfile[64] = "../../tests/wan.bin";
...
void wanUnpack(char *blob)
{
	...
	if(blob != NULL)
	{
		...
		if(unpack_ret == MSGPACK_UNPACK_SUCCESS)
		{
			...
			if(wd != NULL)
			{
				printf("wd->subdoc_name is %s\n", wd->subdoc_name);
				printf("wd->version is %lu\n", (long)wd->version);
				printf("wd->transaction_id %lu\n",(long) wd->transaction_id);
				printf("wd->param->enable %s\n", (1 == wd->param->enable)?"true":"false");
...
void add_suites( CU_pSuite *suite )
{
    *suite = CU_add_suite( "tests", NULL, NULL );
    CU_add_test( *suite, "test_wan_unpack", test_wan_unpack);
}
