...
{
	return comp_helper_convert( buf, len, sizeof(advsecuritydoc_t), "advsecurity", 
...
const char* advsecuritydoc_strerror( int errnum )
{
    ...
    if( NULL == map[i].txt )
    {
	WebcfgDebug("----advsecuritydoc_strerror----\n");
...
//To access the variable arguments use va_list 
	...
	ad->param = (advsecurityparam_t *) malloc( sizeof(advsecurityparam_t) );
        if( NULL == ad->param )
        {
	...
	{
		WebcfgDebug("process_advsecurityparams failed\n");
		return -1;
	}
